...
| Типовой гейт `pyright` заведён на легаси-коде и закрыт подавлениями в 24 местах шести файлов: своих два — динамический `Literal` из enum (`# pyright: reportInvalidTypeForm=false` файлом `ssclient/domain/record_entities.py`) и сужение `Optional` (`# pyright: ignore` в `s2ctl/cmd_sshkey.py`), остальные 22 — построчные `# type: ignore` эпохи mypy: `s2ctl/cmd_domain.py` 13, `ssclient/domain/record.py` 7, `s2ctl/config.py` 1, `s2ctl/context.py` 1 | гейт зелёный, но 24 места типами не описаны, и легаси-подавления `# type: ignore` pyright принимает наравне со своими: они молча делают работу гейта, а по коду не видно, что подавляют | снять свои два вместе с переработкой типов записей DNS; легаси `# type: ignore` разобрать пофайлово и заменить на адресный `# pyright: ignore[...]` или на типы — отдельной карточкой |
