...
- Pipeline: `.gitlab-ci.yml` репозитория (внешних `include` нет). Стадия `test`
  ...
  и `prod` (тег `vX.Y.Z`) собирают бинари Linux и Windows тем же
  `bundle/build_linux.sh` и образами из `docker/`.
...
| Windows-сборка (`docker/Dockerfile.win.py311`, job'ы `build_windows`/`prod_build_windows`) локальными гейтами не воспроизводится: раннера с Windows нет ни у разработчика, ни в проверке фазы | поломка образа или скрипта сборки под Windows обнаруживается только запуском пайплайна на Windows-раннере | держать `bundle/build_linux.sh` и `bundle/build_win.ps1` единственными точками сборки, образ — только окружением для них; шаги окружения Linux-сборки продублированы в job'е `.build_nix` и в `docker/Dockerfile.linux.py311` и правятся парой |
