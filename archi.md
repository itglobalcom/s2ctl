...
| `return_task=true` publisher принимает у любой DELETE-операции, а CLI шлёт его только у трёх новых команд (`gateway delete`, `gateway delete-nic`, `affinity-group delete`) | у остальных удалений (`domain delete`, `domain delete-record`, `server delete` и прочие удаления группы `server`, `network delete`, `ssh-key delete`, удаление тегов) нет ни `--wait`, ни id задачи в выводе: дождаться удаления и связать его с задачей скрипт не может, остаётся опрос списка. Проверено на живом API: `domain delete <zone> --wait` → `Error: No such option '--wait'`, при этом `DELETE /api/v1/domains/{name}?return_task=true` задачу возвращает | добавить `return_task=true` и `--wait` остальным удалениям отдельной карточкой: это меняет поверхность существующих команд — новая опция и другое тело вывода, — а TSK0003840 запрещено критерием приёмки менять существующие команды |
...
| `echo` (`src/s2ctl/click.py`) печатает только истинное значение, поэтому пустая коллекция от API уходит в пустой вывод, а не в `[]`: `gateway get-firewall` на шлюзе без правил печатает ноль байт | скрипт, разбирающий вывод как JSON, получает на пустом наборе пустую строку вместо валидного `[]` и обязан этот случай знать. Со стороны потребителя закрыто: `--rules-file` принимает пустой файл как пустой набор, поэтому round-trip набора правил замыкается и на нём | печать `[]` меняет вывод существующих команд, что критерий приёмки TSK0003840 запрещает, — отдельной карточкой |
