...
| --- | --- | --- | --- |
| ctx_cli_s2ctl | team/arch-owner | s2ctl | src/s2ctl, src/ssclient, tests, bundle, docker |
...
| Пайплайн описан внешним шаблоном `b2c/ci` (`s2ctl/pipeline.yml`, `ref: v1.0.0`), несовместимым со стеком после подъёма зависимостей: `image: python:3.8`, `POETRY_VERSION: "1.0.*"`, `.build_nix` с `poetry install --no-dev`. Шаблон прибит и к именам файлов репозитория: job Windows-сборки делает `docker build -f docker/Dockerfile.win.py38`, а файл в репозитории — `docker/Dockerfile.win.py311` | локальные гейты зелёные, а пайплайн MR не поставит зависимости и не соберёт бинарь: сборка под Linux упадёт на `poetry install`, под Windows — на отсутствующем файле. Правка лежит вне этого репозитория, а переименовывать образ обратно нельзя: имя обязано называть версию Python, на которой он собран | развилка за владельцем: бампнуть шаблон в проекте `b2c/ci` отдельной карточкой либо перенести пайплайн в `s2ctl`. Перенос закрывает и версии, и имена файлов сразу. До этого гейтом остаётся локальный прогон |
