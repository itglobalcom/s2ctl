...
| Идентификатор локации vStack остаётся голым `str` в сигнатурах команд и сервисов (`gateway create`, `gateway list`, `affinity-group create`, `affinity-group list`, `server create`, `server price`, `network create`, фильтры каталогов `images` и `applications`) | составные id раздела закрыты типами с фабрикой, а локация — нет: в заказе сервера рядом стоит такой же `str` `image_id`, и перестановку аргументов типы не ловят | завести общий тип идентификатора локации vStack и провести его по всем командам и сервисам раздела разом — отдельной карточкой. Типизация одних только новых команд оставила бы в разделе два способа обращения с одним идентификатором |
