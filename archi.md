...
| `ServerId` (`l{n}s{n}`) доменного типа не имеет: id сервера ходит в сигнатурах строкой, а у VMware — `int` | нарушение GR-02: формат id проверяет только API, перепутать местами id сервера и сети типы не мешают | завести тип с фабрикой и мигрировать группу `server` (28 команд) отдельной карточкой: правка механическая, но задевает весь раздел |
