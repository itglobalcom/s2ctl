...
async def test_delete_without_wait_sends_bare_delete(fake_http_client, gateway_id):
    fake_http_client.on('DELETE', NIC_PATH, None)

    await GatewayNicService(fake_http_client, gateway_id).delete(7)

    assert fake_http_client.requests == [FakeRequest('DELETE', NIC_PATH)]
...
async def test_delete_with_wait_asks_for_task_and_polls_it(fake_http_client, gateway_id):
    ...
    await GatewayNicService(fake_http_client, gateway_id).delete(7, wait=True)
