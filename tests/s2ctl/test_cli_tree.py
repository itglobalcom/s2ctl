...
# своего типа, правило NAT — адрес, который платформа за него не подставляет,
# а установка автодополнения — оболочку из окружения прогона.
_EXTRA_ARGS = types.MappingProxyType({
    ...
    'vmware edge upsert-nat-rule': ('--original-ip', '10.0.0.1'),
})
